...
async def update_preferences(
    ...
    """
    logger.info("profile_service: updating preferences", extra={"user_id": user_id})

    existing = user_store.get_preferences(user_id) or Preferences()
    ...
    user_store.save_preferences(user_id, merged)
...
async def update_constraints(
    ...
    user_id comes from the authenticated Google user, not the request.
    """
    logger.info("profile_service: updating constraints", extra={"user_id": user_id})

    existing = user_store.get_constraints(user_id) or Constraints()
    ...
    user_store.save_constraints(user_id, merged)
